...
export default function AdminOrders() {
  ...
  return (
    <div className="p-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-serif text-walnut-800">Orders</h1>
        <p className="text-walnut-600">Manage customer orders and fulfillment</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <Filter className="w-5 h-5 text-walnut-600" />
          {['all', 'pending', 'paid', 'fulfilled', 'failed'].map((status) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                statusFilter === status
                  ? 'bg-walnut-800 text-cream-50'
                  : 'bg-white text-walnut-600 hover:bg-dusty-rose-100'
              }`}
            >
      ...
      {filteredOrders.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center">
          ...
          <h3 className="text-xl font-serif text-walnut-800 mb-2">No orders found</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  <th className="text-left p-4 font-medium text-walnut-600">Order</th>
                  <th className="text-left p-4 font-medium text-walnut-600">Customer</th>
                  <th className="text-left p-4 font-medium text-walnut-600">Date</th>
                  <th className="text-left p-4 font-medium text-walnut-600">Total</th>
                  <th className="text-left p-4 font-medium text-walnut-600">Status</th>
                  <th className="text-right p-4 font-medium text-walnut-600">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {filteredOrders.map((order) => (
                  ...
                  >
                    <td className="p-4">
                      <span className="font-mono text-sm text-walnut-800">
                        #{order.id.slice(0, 8).toUpperCase()}
                      </span>
                    </td>
                    <td className="p-4">
                      <div>
                        <p className="font-medium text-walnut-800">{order.customer_name || 'N/A'}</p>
                        <p className="text-sm text-walnut-500">{order.customer_email}</p>
                      </div>
                    </td>
                    <td className="p-4 text-walnut-600 text-sm">
                      {formatDate(order.created_at)}
                    </td>
                    <td className="p-4 font-medium text-walnut-800">
                    ...
                    <td className="p-4 text-right">
                      <button
                        ...
                        className="text-dusty-rose-600 hover:underline text-sm"
                      >
                        View
                      </button>
                    </td>
