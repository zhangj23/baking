...
const sendOrderConfirmation = async (order) => {
  ...
  const emailHtml = `
    ...
    <head>
      <style>
        body { font-family: 'Georgia', serif; background-color: #FFFDD0; margin: 0; padding: 20px; }
        .container { max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        .header { background: linear-gradient(135deg, #E6C2BF 0%, #d4a5a0 100%); padding: 30px; text-align: center; }
        .header h1 { color: #5D4037; margin: 0; font-size: 28px; }
        .content { padding: 30px; color: #5D4037; }
        ...
        .item { padding: 8px 0; border-bottom: 1px solid #E6C2BF; }
        .total { font-weight: bold; font-size: 18px; margin-top: 15px; padding-top: 15px; border-top: 2px solid #5D4037; }
        .pickup-info { background: #5D4037; color: white; padding: 20px; border-radius: 8px; margin-top: 20px; }
        .pickup-info h3 { margin-top: 0; color: #E6C2BF; }
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p>With warmth,<br><strong>The ML Baking Team</strong></p>
        </div>
        <div class="footer">
          <p>© ${new Date().getFullYear()} ML Baking. Made with love and flour.</p>
        </div>
      </div>
    </body>
    </html>
  ...
  const textContent = `
Thank You for Your Order at ML Baking!
...
The ML Baking Team
  ...
  const params = {
    ...
    Message: {
      Subject: {
        Data: `Order Confirmed! 🥐 ML Baking #${order.id.slice(0, 8)}`,
        Charset: 'UTF-8'
      },
      Body: {
        Text: {
